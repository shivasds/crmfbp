...
class Admin extends CI_Controller {
	...
	public function email_report() {
		...
		if($reportType != 'dailyCallback'){
			...
			$tmparry[] = 'ranjeet.singh@propsolutionservices.com';
		...
		if($report_data){
			...
			$this->email->from("ranjeet.singh@propsolutionservices.com", "Admin");
	...
	public function save_online_leads(){
		...
		foreach ($checked as $key) {
				...
				if (!($this->callback_model->isexists_callbacks($data)))
					...
					$this->common_model->updateWhere(array('leadid'=>$key));
	}
	?>
	<script type="text/javascript">
		//alert('success');
		location.href="online_leads";
	</script>
	<?php
	}
}
