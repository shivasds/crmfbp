        ...
        class Common_model extends MY_Model {
            ...
            function getFromId($key, $primary_key ='$id',$table_name='$online_leads')
            {
                $this->db->select('*');
                $this->db->from('online_leads');
                $this->db->where('leadid',$key);
                $query = $this->db->get();
            if ( $query->num_rows() > 0 )
            {
                $row = $query->row_array();
            ...
            function is_exist_online_lead($phone)
            {
                $this->db->select('*');
                $this->db->from('online_leads');
